  ...
  return (
    <div className="flex flex-col h-full bg-[#F6F4F8] text-body relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[-20%] left-[-10%] w-[80%] h-[60%] bg-purple-900/5 blur-[120px] rounded-full opacity-40" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[80%] h-[60%] bg-amber-900/5 blur-[120px] rounded-full opacity-30" />
      </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-32 relative z-10">
        <div className="p-6 space-y-10">
          ...
          <motion.div 
            ...
            className="relative p-10 rounded-[3rem] bg-white border border-black/5 shadow-sm overflow-hidden flex flex-col items-center text-center group"
          >
            ...
            <div className="w-20 h-20 rounded-[2rem] bg-black/5 flex items-center justify-center border border-black/5 mb-6 relative z-10 shadow-inner group-hover:scale-110 transition-transform duration-500">
              <Wallet className="w-10 h-10 text-indigo-600/80" />
            </div>
            
            <div className="relative z-10 space-y-2">
              <p className="text-[10px] font-black text-muted uppercase tracking-[0.3em]">Mevcut Jeton Bakiyesi</p>
              <div className="flex items-center justify-center gap-3">
                <span className="text-6xl font-serif font-bold text-heading tracking-tighter">{currentUser.credits}</span>
                <motion.div
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-serif font-bold text-heading tracking-tight">Premium Paketler</h2>
              </div>
              <span className="text-[10px] font-bold text-muted uppercase tracking-widest">Özel Teklifler</span>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              {config.subscriptions.map((sub, idx) => (
                <motion.div 
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="relative p-8 rounded-[2.5rem] bg-white border border-black/5 shadow-sm overflow-hidden group"
                >
                  {/* Decorative Background Elements */}
                  <div className={`absolute top-0 right-0 w-64 h-64 ${sub.type === 'yearly' ? 'bg-amber-500/5' : 'bg-indigo-500/5'} rounded-full blur-[100px] -mr-32 -mt-32 group-hover:opacity-100 opacity-60 transition-opacity`} />
                  
                  <div className="relative z-10 flex flex-col gap-8">
                    <div className="flex justify-between items-start">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <h3 className="text-2xl font-serif font-bold tracking-tight text-heading">{sub.name}</h3>
                          {sub.type === 'yearly' && (
                        ...
                        <p className="text-xs text-body font-medium tracking-wide">Sınırsız beğeni ve mistik ayrıcalıklar</p>
                      </div>
                      <div className="text-right">
                        <div className="flex items-center gap-1 justify-end">
                          <span className="text-3xl font-serif font-bold text-amber-600">{sub.price}</span>
                          <Sparkles className="w-4 h-4 text-amber-600" />
                        </div>
                        <p className="text-[9px] text-muted font-black uppercase tracking-widest">Jeton / {sub.type === 'monthly' ? 'Ay' : 'Yıl'}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {[
                        ...
                        <div key={fIdx} className="flex items-center gap-3 text-sm text-body font-medium">
                          <div className="w-5 h-5 rounded-full bg-black/5 border border-black/5 flex items-center justify-center shrink-0">
                    ...
                    <button 
                      ...
                      className={`w-full py-5 rounded-2xl ${sub.type === 'yearly' ? 'bg-amber-600 text-white' : 'bg-heading text-white'} font-black text-sm shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100`}
                    >
                      {purchasing === sub.id ? (
